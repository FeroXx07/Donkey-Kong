...
Level2Win::Level2Win(bool startEnabled) : Module(startEnabled)
{
	level2win.x = 240;
	level2win.y = 160;
	level2win.w = SCREEN_WIDTH;
	level2win.h = SCREEN_HEIGHT;
...
update_status Level2Win::Update()
{
...
update_status Level2Win::PostUpdate()
{
	LOG("Drawing background assets of the WinScene");
	if (frameCount <= 240)
	{
		App->render->Blit(bgTexture, 0, 0, &level2win);
